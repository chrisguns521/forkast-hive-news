...
const PredictionCard: React.FC<PredictionCardProps> = ({ prediction }) => {
  return (
    <Card className="overflow-hidden mb-4 bg-gray-900 text-white border-none">
      <div className="relative aspect-[4/3] w-full overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <div className="flex justify-between items-start">
            <h3 className="text-base font-bold leading-tight">{prediction.question}</h3>
            <button className="text-gray-300 hover:text-forkast-400 mt-0.5">
              <Star className={`h-5 w-5 ${prediction.favorited ? 'fill-forkast-400 text-forkast-400' : ''}`} />
            </button>
          </div>
        </div>
      </div>
      
      <CardContent className="p-3 pt-2 bg-gray-900">
        <div className="grid grid-cols-2 gap-2 mb-3">
          <Button 
            variant="ghost" 
            className="bg-emerald-900/50 hover:bg-emerald-800/70 text-white flex items-center justify-center py-1.5 h-auto"
          >
            <Check className="mr-1.5 h-4 w-4" /> Yes
          </Button>
          <Button 
            variant="ghost" 
            className="bg-red-900/50 hover:bg-red-800/70 text-white flex items-center justify-center py-1.5 h-auto"
          >
